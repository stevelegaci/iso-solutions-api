...
STRIPE_SECRET_KEY      = os.getenv("STRIPE_SECRET_KEY", "sk_test_51SI4tj3wAwm7WSvkp3DodIunn9m9iHIvZeBpt6FLssZARmJeq8Qu7bAiyKeOZeCxKs8fmNHoUj3CELW2GER9O9vQ00RhrGADfp")
STRIPE_WEBHOOK_SECRET  = os.getenv("STRIPE_WEBHOOK_SECRET", "")
DASHBOARD_URL          = os.getenv("DASHBOARD_URL", "http://localhost:3000")

PRICE_MAP = {
    "paid":    "price_1TgPCh3wAwm7WSvkQNyajoEe",
    "premium": "price_1TgPCi3wAwm7WSvk838rKU8b",
}
...
async def stripe_webhook(
    ...
    if etype in ("customer.subscription.created", "customer.subscription.updated"):
        ...
        plan        = PLAN_FROM_PRICE.get(price_id, "paid")
        sub_id      = sub["id"]
        conn = get_db()
        conn.execute(
            "UPDATE members SET plan=?, stripe_subscription_id=? WHERE stripe_customer_id=?",
        ...
        conn = get_db()
        conn.execute(
            "UPDATE members SET plan='free', stripe_subscription_id=NULL WHERE stripe_customer_id=?",
            (customer_id,)
        )
        conn.commit()
        conn.close()
        ...
        plan        = sess.get("metadata", {}).get("plan", "paid")
        customer_id = sess.get("customer")
        if member_id:
            conn = get_db()
            conn.execute(
                "UPDATE members SET plan=?, stripe_customer_id=? WHERE id=?",
                (plan, customer_id, member_id)
            )
            conn.commit()
            conn.close()
...
async def verify_session(session_id: str, member_id: Optional[str] = None):
    ...
    if session.payment_status in ("paid", "no_payment_required") and member_id:
        ...
        conn = get_db()
        conn.execute(
            "UPDATE members SET plan=?, stripe_customer_id=?, stripe_subscription_id=? WHERE id=?",
            (plan, customer_id, sub_id, member_id)
        )
        conn.commit()
        conn.close()
